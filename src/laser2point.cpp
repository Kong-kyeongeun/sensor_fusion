...
void laser2point::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan){
    sensor_lsc_ = *scan;
    sub_laser= true;
    tf_ls_.waitForTransform(sensor_lsc_.header.frame_id, "/laser_frame",
        sensor_lsc_.header.stamp + ros::Duration().fromSec(sensor_lsc_.ranges.size() * sensor_lsc_.time_increment), ros::Duration(0.05));
  
    projector_.transformLaserScanToPointCloud("/laser_frame" , sensor_lsc_, sensor_pcl_, tf_ls_);
}
...
void laser2point::pub_pointcloud(){
    if(sub_laser){
        pub_point.publish(sensor_pcl_output);
    }
}
